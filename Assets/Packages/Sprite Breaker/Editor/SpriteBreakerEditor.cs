...
public class SpriteBreakerEditor : EditorWindow {
    ...
    void OnGUI () {
        ...
        if ( currentSprite != null ) {
            ...
            // set cursor
            if ( _currentAction == EditAction.Pan ) EditorGUIUtility.AddCursorRect( view, UnityEditor.MouseCursor.Pan );
            else if ( _currentAction == EditAction.Move ) {
                if ( moveMode == MoveMode.Move ) EditorGUIUtility.AddCursorRect( view, UnityEditor.MouseCursor.MoveArrow );
                else if ( moveMode == MoveMode.Scale ) EditorGUIUtility.AddCursorRect( view, UnityEditor.MouseCursor.ScaleArrow );
                else if ( moveMode == MoveMode.Rotate ) EditorGUIUtility.AddCursorRect( view, UnityEditor.MouseCursor.RotateArrow );
            } else if ( _currentAction == EditAction.Select && currentEvent.shift ) EditorGUIUtility.AddCursorRect( view, UnityEditor.MouseCursor.ArrowPlus );
            else if ( _currentAction == EditAction.Create && ( currentData.polygons.Count > 2 && _mouseOverVertexPoly == currentData.polygons.Count - 1 && _mouseOverVertex == 0 ) ) EditorGUIUtility.AddCursorRect( view, UnityEditor.MouseCursor.Link );
            else if ( _editMode == EditMode.Edit && _currentAction == EditAction.None && currentEvent.alt && _mouseOverVertex >= 0 ) EditorGUIUtility.AddCursorRect( view, UnityEditor.MouseCursor.ArrowMinus );
